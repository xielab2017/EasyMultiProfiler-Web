...
.llm_call_curl <- function(url, headers, body, timeout = 120) {
  ...
  if (nzchar(py)) {
    ...
    if (nzchar(err)) {
      # Fall through to curl below; keep Python error if curl is unavailable.
      py_err <- err
    } else {
  ...
  if (!nzchar(Sys.which("curl"))) stop(sprintf("No HTTP client available for LLM calls: %s", py_err))
  args <- c(
  ...
  err <- paste(readLines(err_file, warn = FALSE), collapse = "\n")
  txt <- paste(readLines(out_file, warn = FALSE), collapse = "\n")
  if (!identical(status, 0L)) stop(sprintf("LLM request failed: %s\n%s", py_err, err %||% status))
...
optimize_r_with_llm <- function(provider, cfg, code, workflow = NULL, tab = NULL, instruction = NULL, ui_context = NULL) {
  ...
  if (identical(provider, "auto")) {
    ...
    if (!length(providers)) providers <- c("deepseek", "qwen", "chatgpt", "claude", "gemini", "minimax")
  } else {
    providers <- provider
  }
  errors <- character(0)
