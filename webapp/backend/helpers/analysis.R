...
run_alpha <- function(session_id, experiment, method = "shannon", source = "current") {
  ...
  empt_alpha <- tryCatch(
    empt |> EasyMultiProfiler::EMP_alpha_analysis(),
    error = function(e) {
      message("EMP_alpha_analysis fallback (vegan): ", conditionMessage(e))
      empt
    }
  )
  if (!identical(source_key, "raw") && !identical(empt_alpha, empt)) {
    save_empt(session_id, experiment, empt_alpha)
    empt <- empt_alpha
  }
  ...
  if (nrow(X) && requireNamespace("vegan", quietly = TRUE)) {
    ...
    est <- if (is_count_like) suppressWarnings(vegan::estimateR(X)) else NULL
    sid <- rownames(result)
...
.diff_run_native_multi <- function(session_id, experiment, empt, group_var, method_raw,
  ...
  bump(92, "Caching DEG table for visualization")
  tryCatch({
    ref_g <- NULL
    test_g <- NULL
    if ("vs" %in% names(result) && nrow(result)) {
      vs0 <- unique(as.character(result$vs))[1]
      if (length(vs0) && nzchar(vs0)) {
        test_g <- sub(" vs .*", "", vs0)
        ref_g  <- sub(".* vs ", "", vs0)
      }
    }
    raw_df <- .diff_compact_to_raw(result, empt,
                                   ref_group = ref_g, test_group = test_g,
                                   group_var = group_var)
    .save_diff_raw_cache(session_id, experiment, native_method, group_var,
                         ref_g, test_g, raw_df,
                         comparison_mode = comparison_mode)
  }, error = function(e) {
    message("[diff] raw result cache skipped (multi): ", conditionMessage(e))
  })
  bump(100, "Done")
  result
}
...
run_diff <- function(session_id, experiment, method = "DESeq2",
  ...
  tryCatch({
    raw_df <- .compute_diff_raw(session_id, experiment,
                                method = method_use,
                                group_var = group_var, group_var_orig = group_var,
                                ref_group = ref_group, test_group = test_group,
                                compact = result)
    .save_diff_raw_cache(session_id, experiment, method_use, group_var,
                         ref_group, test_group, raw_df,
                         comparison_mode = "pairwise")
  }, error = function(e) {
...
# ---------------------------------------------------------------------------
.diff_resolve_group_col <- function(cd, group_var) {
  if (!is.null(group_var) && nzchar(group_var) && group_var %in% names(cd)) {
    return(group_var)
  }
  for (alt in c(
    paste0(group_var, "_safe"),
    paste0(group_var, "__emp")
  )) {
    if (alt %in% names(cd)) return(alt)
  }
  NA_character_
}

.diff_pick_result_col <- function(df, patterns) {
  for (pat in patterns) {
...
.diff_compact_to_raw <- function(compact, empt, ref_group = NULL, test_group = NULL,
                                 group_var = NULL) {
  if (is.null(compact) || !nrow(compact)) return(NULL)
  df <- as.data.frame(compact, stringsAsFactors = FALSE)
  if ("vs" %in% names(df)) {
    if (!is.null(ref_group) && !is.null(test_group)) {
      vs_want <- c(
        paste0(test_group, " vs ", ref_group),
        paste0(ref_group, " vs ", test_group)
      )
      sub <- df[df$vs %in% vs_want, , drop = FALSE]
      if (nrow(sub)) df <- sub
    } else {
      vs0 <- unique(as.character(df$vs))[1]
      if (length(vs0) && nzchar(vs0)) {
        df <- df[df$vs == vs0, , drop = FALSE]
      }
    }
  }
  if ("feature" %in% names(df)) {
    df <- df[!duplicated(df$feature), , drop = FALSE]
  }
  ad <- SummarizedExperiment::assays(empt)[[1]]
  feats <- if ("feature" %in% names(df)) as.character(df$feature) else rownames(df)
  fc_col <- .diff_pick_result_col(df, c("^log2fc$", "^log2foldchange$", "^lfc$"))
  p_col  <- .diff_pick_result_col(df, c("^pvalue$", "^p_value$", "^p\\.value$"))
  q_col  <- .diff_pick_result_col(df, c("^padj$", "^fdr$", "^q[._]?value$", "^adj[._]?p"))
  baseMean <- tryCatch({
    common <- intersect(feats, rownames(ad))
    bm <- rep(NA_real_, length(feats))
    if (length(common)) {
      bm[match(common, feats)] <- rowMeans(ad[common, , drop = FALSE], na.rm = TRUE)
    }
    bm
  ...
  data.frame(
    ...
    symbol         = feats,
    baseMean       = baseMean,
    log2FoldChange = if (!is.na(fc_col)) suppressWarnings(as.numeric(df[[fc_col]])) else NA_real_,
    lfcSE          = NA_real_,
    stat           = NA_real_,
    pvalue         = if (!is.na(p_col)) suppressWarnings(as.numeric(df[[p_col]])) else NA_real_,
    padj           = if (!is.na(q_col)) suppressWarnings(as.numeric(df[[q_col]])) else NA_real_,
    stringsAsFactors = FALSE
  )
}

.save_diff_raw_cache <- function(session_id, experiment, method, group_var,
                                 ref_group, test_group, raw_df,
                                 comparison_mode = "pairwise") {
  if (is.null(raw_df) || !nrow(raw_df)) return(invisible(NULL))
  saveRDS(list(
    method = method,
    group_var = group_var,
    ref_group = ref_group,
...
.compute_diff_raw <- function(session_id, experiment,
  ...
  if (is_deseq2 && requireNamespace("DESeq2", quietly = TRUE)) {
    gv_col <- .diff_resolve_group_col(cd, group_var_orig %||% group_var)
    if (is.na(gv_col)) stop("Grouping variable not found in sample metadata.")
    sub_idx <- which(as.character(cd[[gv_col]]) %in% c(ref_group, test_group))
    ...
    coldat[[gv_col]] <- factor(as.character(coldat[[gv_col]]),
    ...
    dds <- DESeq2::DESeqDataSetFromMatrix(countData = counts,
                                          colData   = coldat,
                                          design    = as.formula(paste0("~", gv_col)))
    dds <- DESeq2::DESeq(dds, parallel = FALSE, quiet = TRUE)
    res <- DESeq2::results(dds, contrast = c(gv_col, test_group, ref_group))
    raw <- data.frame(
      feature        = rownames(res),
      symbol         = rownames(res),
  ...
  raw <- .diff_compact_to_raw(compact, empt, ref_group = ref_group,
                              test_group = test_group, group_var = group_var_orig)
  if (is.null(raw) || !nrow(raw)) stop("No differential result available to build raw table.")
  raw
}
...
# Load cached raw DEG table, or rebuild from the EMPT diff deposit when the
# cache was skipped (e.g. older sessions or multi-group runs before v5.0.1).
ensure_diff_raw <- function(session_id, experiment) {
  raw <- load_diff_raw(session_id, experiment)
  if (!is.null(raw) && !is.null(raw$data) && nrow(raw$data)) return(raw)
  empt <- tryCatch(load_empt(session_id, experiment), error = function(e) NULL)
  if (is.null(empt)) return(NULL)
  compact <- tryCatch(empt@deposit[["diff_analysis_result"]], error = function(e) NULL)
  if (is.null(compact) || !nrow(compact)) {
    compact <- tryCatch(
      as.data.frame(EasyMultiProfiler::EMP_result(empt, info = "diff_analysis_result"),
                     stringsAsFactors = FALSE),
  ...
  if (is.null(compact) || !nrow(compact)) return(NULL)
  gv <- tryCatch(empt@metadata$estimate_group, error = function(e) NULL)
  if (is.null(gv) || !nzchar(gv)) {
    gv <- if ("Estimate_group" %in% names(compact)) compact$Estimate_group[1] else NULL
  }
  ref_g <- NULL
  test_g <- NULL
  if ("vs" %in% names(compact)) {
    vs0 <- unique(as.character(compact$vs))[1]
    if (length(vs0) && nzchar(vs0)) {
      test_g <- sub(" vs .*", "", vs0)
      ref_g  <- sub(".* vs ", "", vs0)
    }
  }
  method <- tryCatch(empt@metadata$method, error = function(e) "EMP")
  raw_df <- .diff_compact_to_raw(compact, empt, ref_group = ref_g,
                                 test_group = test_g, group_var = gv)
  ...
  tryCatch(saveRDS(raw, diff_raw_path(session_id, experiment)), error = function(e) NULL)
  raw
}
...
run_enrichment <- function(session_id, experiment,
  ...
  db_input <- toupper(trimws(as.character(database %||% "KEGG")))
  go_ont <- switch(
    db_input,
    GO_CC = "CC",
    GO_MF = "MF",
  ...
  method_use  <- if (startsWith(db_input, "GO")) "go" else tolower(db_input)
  ...
  if (is.null(res)) {
    ...
    return(list(
      success = TRUE,
      database = if (method_use == "go") paste0("GO_", go_ont) else toupper(method_use),
      organism = org_use,
      ...
      if (requireNamespace("enrichplot", quietly = TRUE)) {
        p <- enrichplot::dotplot(res, showCategory = min(as.integer(top_n), 20)) +
          ggplot2::labs(title = paste0(
            if (method_use == "go") paste0("GO-", go_ont) else toupper(method_use),
            " enrichment (",
