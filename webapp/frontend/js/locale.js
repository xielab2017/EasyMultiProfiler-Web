 ...
 */
import { I18N_CATALOG } from "./i18n_catalog.js?v=2026-06-20-v5.0.0";
import { applyDomI18n } from "./ui_dom_i18n.js?v=2026-06-20-v5.0.0";
...
function applyCourseBanner() {
  ...
  const loc = getLocale();
  if (loc === "en") {
    root.innerHTML = `<strong>v5.0.0</strong> · New users: see <button type="button" class="btn-link" id="btn-course-to-guide">Guide</button> in the sidebar (Mac vs Windows differ) · One-click demo + recommended defaults per lesson`;
  } else {
    root.innerHTML = `<strong>v5.0.0</strong> · 零基础请先看左侧 <button type="button" class="btn-link" id="btn-course-to-guide">Guide 安装指南</button>（Mac 与 Windows 安装方式不同）· 每课可「一键加载示例数据」+「套用推荐参数」`;
  }
